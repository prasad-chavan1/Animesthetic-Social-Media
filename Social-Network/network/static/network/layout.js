...
function login_popup(action) {
  ...
  if (action === "like") {
    ...
    document.querySelector(".icon-div").innerHTML = `
        <svg width="2.5em" height="2.5em" viewBox="0 0 16 16" class="bi bi-bookmark-fill" fill="#d92b2e" xmlns="http://www.w3.org/2000/svg">
    ...
    document.querySelector(".icon-div").innerHTML = `
        <svg width="2.5em" height="2.5em" viewBox="0 0 24 24" fill="#d92b2e" class="r-1re7ezh r-4qtqp9 r-yyyyoo r-1q142lx r-1xvli5t r-19einr3 r-dnmrzs r-bnwqim r-1plcrui r-lrvibr">
...
function save_post(element) {
  ...
  fetch("/n/post/" + parseInt(id) + "/save", {
    ...
    element.querySelector(".svg-span").innerHTML = `
            <svg width="1.1em" height="1.1em" viewBox="0.5 0 15 15" class="bi bi-bookmark-fill" fill="#d92b2e" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" d="M3 3a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v12l-5-3-5 3V3z"/>
            </svg>`;
...
function follow_user(element, username, origin) {
  ...
  fetch("/" + username + "/follow", {
    ...
    if (origin === "suggestion") {
      element.parentElement.innerHTML = `<button class="btn btn-danger" type="button" onclick="unfollow_user(this,'${username}','suggestion')">Following</button>`;
    } else if (origin === "edit_page") {
      element.parentElement.innerHTML = `<button class="btn btn-danger float-right" onclick="unfollow_user(this,'${username}','edit_page')" id="following-btn">Following</button>`;
...
function unfollow_user(element, username, origin) {
  ...
  fetch("/" + username + "/unfollow", {
    ...
    if (origin === "suggestion") {
      element.parentElement.innerHTML = `<button class="btn btn-outline-danger" type="button" onclick="follow_user(this,'${username}','suggestion')">Follow</button>`;
    } else if (origin === "edit_page") {
      element.parentElement.innerHTML = `<button class="btn btn-outline-danger float-right" onclick="follow_user(this,'${username}','edit_page')" id="follow-btn">Follow</button>`;
    } else if (origin === "dropdown") {
      ///////////////////////////////////////////////////////////////////////////////////////////
    }
